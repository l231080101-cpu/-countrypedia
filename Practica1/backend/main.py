...
# 1. CARGA DE CONFIGURACIÓN Y SECRETOS
load_dotenv()
PORT = int(os.getenv('PORT', 5000))
HOST = os.getenv('HOST', '0.0.0.0')
origins = os.getenv('ORIGEN_PERMITIDO', '*').split(',')
...
class Config:
    ...
    # Base de Datos (Prioriza Supabase/Postgres, cae a SQLite en local)
    DATABASE_URL = os.getenv('DATABASE_URL', 'geocultural.db')
...
# ... (tus imports arriba)
from flask_cors import CORS # Asegúrate de tener este import
...
origins = [o.strip() for o in app.config['ORIGEN_PERMITIDO'].split(',')]
...
def require_api_key(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if not api_key or api_key != app.config['API_KEY_PROTECTION']:
...
def token_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        try:
            # Uso del SECRET_KEY centralizado
            data = jwt.decode(token, app.config['SECRET_KEY'], algorithms=['HS256'])
            current_user_id = data['user_id']
            
            # Conexión a DB usando la config
            conn = sqlite3.connect(app.config['DATABASE_URL'])
            cursor = conn.cursor()
            cursor.execute("SELECT id FROM usuarios WHERE id = %s", (current_user_id,))
...
# 4. BASE DE DATOS E INICIO (Mantenemos SQLite por ahora para no romper tu flujo local)
def init_db():
    conn = sqlite3.connect(app.config['DATABASE_URL'])
    cursor = conn.cursor()
    # ... (Aquí van todos tus CREATE TABLE IF NOT EXISTS que ya tienes)
    # [Omitido por brevedad para centrarse en la estructura, pero mantenlos en tu archivo]
    conn.commit()
    conn.close()

# Log de inicio profesional
print(f"✅ Servidor configurado correctamente")
print(f"• Puerto: {app.config['PORT']} | Host: {app.config['HOST']}")
if app.config['OPENWEATHER_API_KEY']:
    print("• Clima: API Key cargada correctamente ✅")

# --- AQUÍ CONTINUARÍAN TUS RUTAS Y LÓGICA DE CACHE ---
exchange_cache = {"data": None, "timestamp": 0}
weather_cache = {}
WEATHER_CACHE_TTL = 1800

print(f"✅ Servidor iniciado con configuración:")
print(f"   • Puerto: {PORT}")
print(f"   • Host: {HOST}")
print(f"   • CORS permitido: {origins}")
print(f"   • API Países: {REST_COUNTRIES_API}")
print(f"   • API Cambio: {EXCHANGE_API}")
print(f"   • Access token expira en: {ACCESS_TOKEN_EXPIRE_MINUTES} minutos")
print(f"   • Refresh token expira en: {REFRESH_TOKEN_EXPIRE_DAYS} días")
if OPENWEATHER_API_KEY:
    print(f"   • Clima: API Key configurada ✅")
else:
    print(f"   • Clima: API Key NO configurada ⚠️")
...
def get_db_connection():
    # Buscamos la URL de Supabase en la configuración
    db_url = app.config.get('DATABASE_URL')
    
    if db_url and db_url.startswith('postgresql'):
        # Si existe la URL, conectamos a Supabase (Postgres)
        return psycopg2.connect(db_url)
    else:
        # Si no, usamos el archivo local
        conn = sqlite3.connect('geocultural.db')
        conn.row_factory = sqlite3.Row
        return conn

def init_db():
    """Crea las tablas si no existen (compatible SQLite/PostgreSQL)"""
    conn = get_db_connection()
    cursor = conn.cursor()
    is_postgres = app.config['DATABASE_URL'].startswith('postgresql')
    
    # Tipo de dato para autoincrementales
    pk_type = "SERIAL PRIMARY KEY" if is_postgres else "INTEGER PRIMARY KEY AUTOINCREMENT"
    bool_default = "FALSE" if is_postgres else "0"
    
    # Lista de comandos SQL para crear tablas
    sql_commands = [
        f"""
...
def get_country_by_cca3(cca3):
    ...
    try:
        ...
        if conn:
            conn.rollback()
        print(f"🔴 Error obteniendo país por cca3 {cca3}: {e}")
    finally:
        if conn:
            try:
                cursor.close()
...
def buscar_pais(name):
    ...
    try:
        ...
        print("🔴 ERROR REAL EN SERVIDOR:", str(e)) # Esto saldrá en tu terminal negra de VS Code
...
def travel_advisory(country_name):
    ...
    try:
        ...
        print(f"⚠️ Error obteniendo región: {e}")
    if not region:
        region = "Americas"
